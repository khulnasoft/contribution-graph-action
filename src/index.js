...
const secondLine =
  "Committed via https://github.com/marketplace/actions/contribution-graph-action";
...
await dayOffsets
  ...
  .map((/** @type {Date} */ day) => {
    const commitsToMake = getRandomInt(
      ...
      env.MAX_COMMITS_PER_DAY
    );
    return [...Array(commitsToMake)].map((_, i) => async () => {
      const { commit: sha } = await git(localPath).commit(
        [env.GIT_COMMIT_MESSAGE, secondLine],
        {
          "--allow-empty": null,
      ...
      console.log(
        `Successfully committed ${sha} on ${day.toISOString()} (${
          i + 1
        } / ${commitsToMake})`
      );
    });
  ...
  .flat()
  .reduce(
    (commitPromises, nextPromise) => commitPromises.then(nextPromise),
    Promise.resolve()
  );
